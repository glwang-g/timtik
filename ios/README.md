...
- 正向计时、秒/分钟倒计时和 3 秒预备。
- 默认每秒短滴、每 5 秒长滴、每 15 秒中文报时；短滴和长滴的周期、音色及自定义音频均可分别配置。
- 停止时按秒记录本地历史。
- 前台自动语音识别：开始计时、倒计时 `N` 秒/分钟、停止；“停止”在中间识别结果即响应。进入后台或锁屏即停止监听。
- Siri / 快捷指令：开始、倒计时、停止。Siri 唤起 App 后将命令交由界面处理。
- 倒计时结束会调度本地通知，因此 App 在后台时仍能提醒。
...
- 后台计时靠开始时间重新计算；倒计时结束由 `UserNotifications` 提醒。
- Siri、真实语音识别、通知权限和签名必须在真机测试。当前 Siri 指令会唤起 App；若要在锁屏下无需打开 App 直接执行，须将计时状态改为可由 App Intent 后台持久化和操作。
